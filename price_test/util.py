...
def encode_data(predicates, joins, column_min_max_vals, column_value_index_dicts, column2vec, op2vec, join2vec):
    predicates_enc = []
    joins_enc = []
    for i, query in enumerate(predicates):
        ...
        for predicate in query:
            if len(predicate) == 3:
                ...
                column = predicate[0]
                operator = predicate[1]
                val = predicate[2].strip("'")
                
                if column_value_index_dicts[column] is not None:
                    val = column_value_index_dicts[column][val]
                    
                if operator == '<=':
                    operator = '<'
                    predicate[1] = '<'
                    predicate[2] = float(val) + 1
